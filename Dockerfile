...
RUN echo 'deb http://ppa.launchpad.net/webupd8team/java/ubuntu trusty main' > /etc/apt/sources.list.d/webupd8team-ubuntu-java.list && \
    ...
    apt-get update && \
    apt-get install -y --force-yes --no-install-recommends software-properties-common sudo openssh-client gnupg2 gnupg-curl hopenpgp-tools file bash-completion psmisc wget curl ca-certificates lsof && \
